...
(function() {
  ...
  var searchFormController =
    ...
    //console.log("_______________HIDDEN");
    ...
    //ftheeten 2020 02 10
    var switchNonExclusive = function(params, keyword) 
    {
        //console.log(params);
        var obj=params;
        obj.or=obj.any;
...
//console.log($scope.searchObj);
      ...
      if($scope.searchObj.exclusive != true)
      {
            //console.log("REPLACE");
            var tmp=switchNonExclusive(finalParams);
            finalParams=tmp;
            $scope.finalParams = tmp;
      }
...
//console.log('__Directive 2');
     ...
     */
    if ($scope.searchObj.permalink) {
    //console.log("___________DIRECTIVE 0")
      var triggerSearchFn = self.triggerSearchFn;
      var facetsParams;

            //ftheeten 2020 03 06
      $scope.drawBBoxOnMap= function()
      {
        console.log("box");
        
       
        xmin=parseFloat($scope.coord.xmin);
        xmax=parseFloat($scope.coord.xmax);
        ymin=parseFloat($scope.coord.ymin);
        ymax=parseFloat($scope.coord.ymax);
         $scope.searchObj.params.geometry= "POLYGON(("+xmin+" "+ymin+","+xmax+" "+ymin+","+xmax+" "+ymax+","+xmin+" "+ymin+","+xmin+" "+ymin+"))";
         console.log($scope.searchObj.params.geometry);
           $scope.searchObj.params.relation="within_bbox";
            $scope.triggerSearch();
      };
       //ftheeten 2020 03 06
       $scope.addPositionControl=function()
       {
            /*console.log("graticule");
            var graticule =  new ol.Graticule();
          $scope.searchObj.searchMap.addLayer(graticule);*/
          ...
          $scope.searchObj.searchMap.addControl(mousePositionControl);
          ...
          graticule.setMap($scope.searchObj.searchMap);
       }
